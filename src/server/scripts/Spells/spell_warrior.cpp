...
enum WarriorSpells
{
	...
	WARRIOR_SPELL_T16_DPS_2P					= 144436,
	WARRIOR_SPELL_T16_DPS_2P_EFFECT				= 144438,
	TRINKET_MULTISTRIKE							= 146061,
	TRINKET_MULTISTRIKE_AURA					= 156059
};
            ...
            
            {
...

                {
                    if (Unit* target = GetHitUnit())
                    {
	                    ...
	                    return;
						SetHitDamage(GetHitDamage() * 0.75);
...
// T16 - 2P
// Mortal Strike - 12294
class spell_warr_t16_2p : public SpellScriptLoader
{
public:
    spell_warr_t16_2p() : SpellScriptLoader("spell_warr_t16_2p") { }

    class spell_warr_t16_2p_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_warr_t16_2p_SpellScript);

        void HandleOnCast()
        {
            if (Unit* l_Caster = GetCaster())
            {
                if (l_Caster->HasAura(WARRIOR_SPELL_T16_DPS_2P))
                    l_Caster->CastSpell(l_Caster, WARRIOR_SPELL_T16_DPS_2P_EFFECT, true);
            }
        }

        void Register() override
        {
            OnCast += SpellCastFn(spell_warr_t16_2p_SpellScript::HandleOnCast);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_warr_t16_2p_SpellScript();
    }
};

class spell_novo_sombra : public SpellScriptLoader
{
public:
	spell_novo_sombra() : SpellScriptLoader("spell_novo_sombra") { }

	class spell_novo_sombra_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_novo_sombra_SpellScript);
		...
		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())
				...
				if (caster->HasAura(21359))
					...
					caster->CastSpell(caster, 142885, true);
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_novo_sombra_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_novo_sombra_SpellScript();
	}
};
