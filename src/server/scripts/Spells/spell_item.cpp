	...
	class spell_item_fusion_fire_core_AuraScript : public AuraScript
	{
		...
		void HandleProc(AuraEffect const* p_AurEff, ProcEventInfo& p_ProcInfo)
		{
            ...
            if (DamageInfo* l_DamageInfos = p_ProcInfo.GetDamageInfo())
            {
				...
				_player->CastCustomSpell(l_Caster, SPELL_CLEAVE_SPREAD, &l_Damages, NULL, NULL, true);
            }
        }
    ...
    public:
        ...
        class spell_item_juggernault_focusing_crystal_AuraScript : public AuraScript
        {
            ...
            void OnProc(AuraEffect const* aurEff, ProcEventInfo& procInfo)
            {
                ...
                {
                    int32 l_Healing = l_DamageInfo->GetDamage() * 1.0f;
...
public:
	...
	class spell_item_multistrike_AuraScript : public AuraScript
	{
		...
		void HandleProc(AuraEffect const* p_AurEff, ProcEventInfo& p_ProcInfo)
		{
			...
			if (DamageInfo* l_DamageInfos = p_ProcInfo.GetDamageInfo())
			{
				int32 l_Damages = l_DamageInfos->GetDamage() / 10;
		...
		void Register()
		{
			OnEffectProc += AuraEffectProcFn(spell_item_multistrike_AuraScript::HandleProc, SpellEffIndex::EFFECT_0, AuraType::SPELL_AURA_DUMMY);
		}
	};
...
// LENDARIA PROC PERIÓDICO
class spell_item_lendariaheal_periodico : public SpellScriptLoader
{
public:
	spell_item_lendariaheal_periodico() : SpellScriptLoader("spell_item_lendariaheal_periodico") { }

	class spell_item_lendariaheal_periodico_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_item_lendariaheal_periodico_AuraScript);

		void OnTick(AuraEffect const* aurEff)
		{
			if (Unit* caster = GetCaster())
			{
				if (caster->HasAura(137248))
				{
					if (roll_chance_i(2))
						caster->CastSpell(caster, 119313, true);
				}
			}
		}
		...
		void Register()
		{
			OnEffectPeriodic += AuraEffectPeriodicFn(spell_item_lendariaheal_periodico_AuraScript::OnTick, EFFECT_0, SPELL_AURA_PERIODIC_HEAL);
		}
	};

	AuraScript* GetAuraScript() const
	{
		return new spell_item_lendariaheal_periodico_AuraScript();
	}
};



class spell_item_lendariaheal_castado : public SpellScriptLoader
{
public:
	spell_item_lendariaheal_castado() : SpellScriptLoader("spell_item_lendariaheal_castado") { }

	class spell_item_lendariaheal_castado_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_item_lendariaheal_castado_SpellScript);

		void HandleOnCast()
		{
			if (Unit* l_Caster = GetCaster())
			{
				if (l_Caster->HasAura(137248))
				{
					if (roll_chance_i(4))
						l_Caster->CastSpell(l_Caster, 119313, true);
				}
			}
		}
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_item_lendariaheal_castado_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_item_lendariaheal_castado_SpellScript();
	}
};

class spell_item_lendariadps : public SpellScriptLoader
{
public:
	spell_item_lendariadps() : SpellScriptLoader("spell_item_lendariadps") { }

	class spell_item_lendariadps_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_item_lendariadps_SpellScript);

		void HandleOnCast()
		{
			if (Unit* l_Caster = GetCaster())
			{
				if (l_Caster->HasAura(137595))
				{
					if (roll_chance_i(5))
						l_Caster->CastSpell(l_Caster, 144234, true);
				}
			}
		}
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_item_lendariadps_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_item_lendariadps_SpellScript();
	}
};
...
{
	new spell_item_lendariadps();
	new spell_item_lendariaheal_castado();
	new spell_item_lendariaheal_periodico();
