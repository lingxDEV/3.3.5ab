...
enum RogueSpells
{
	...
	ROGUE_T16_AURA								 = 145185,
	ROGUE_T16_SPELL								 = 145193,
	ROGUE_T16_AURA_4P							 = 145210,
	ROGUE_T16_SPELL_4P							 = 145249,
                    ...
                    if (Unit* target = GetHitUnit())
                    {
                        if (comboPoints)
                        {
                            ...
                            if (_player->HasSpellCooldown(ROGUE_SPELL_SHADOW_BLADES))
                                _player->ReduceSpellCooldown(ROGUE_SPELL_SHADOW_BLADES, (1 * comboPoints * 4000));

                            if (_player->HasSpellCooldown(ROGUE_SPELL_SPRINT))
                                _player->ReduceSpellCooldown(ROGUE_SPELL_SPRINT, (1 * comboPoints * 3000));

                            if (_player->HasSpellCooldown(ROGUE_SPELL_VANISH))
                                _player->ReduceSpellCooldown(ROGUE_SPELL_VANISH, (1 * comboPoints * 3000));
                ...
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (Aura* recuperate = _player->GetAura(ROGUE_SPELL_RECUPERATE))
                    {
                        ...
                        if (_player->HasAura(ROGUE_GLYPH_OF_RECUPERATE))
                            bp += _player->CountPctFromMaxHealth(2);
                ...
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (_player->HasAura(ROGUE_GLYPH_OF_VANISH))
                    {
                        if (Aura* vanishImmunity = _player->GetAura(ROGUE_SPELL_VANISH_IMMUNITY))
                        {
                            ...
                            vanishImmunity->SetDuration(Duration + 7000);
                        }
                    }
                }
            }
...
class spell_item_rogue_t16_2p : public SpellScriptLoader
{
public:
	spell_item_rogue_t16_2p() : SpellScriptLoader("spell_item_rogue_t16_2p") { }

	class spell_item_rogue_t16_2p_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_item_rogue_t16_2p_SpellScript);

		void HandleOnHit()
		{
			if (Player* _player = GetCaster()->ToPlayer())
				if (Unit* target = GetHitUnit())
					if (_player->HasAura(ROGUE_T16_AURA))
						_player->CastSpell(target, ROGUE_T16_SPELL, true);
		}

		void Register()
		{
			OnHit += SpellHitFn(spell_item_rogue_t16_2p_SpellScript::HandleOnHit);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_item_rogue_t16_2p_SpellScript();
	}
};
...
public:
	...
	class spell_item_rogue_t16_4p_SpellScript : public SpellScript
	{
		...
		void HandleOnHit()
		{
			if (Player* _player = GetCaster()->ToPlayer())
				if (Unit* target = GetHitUnit())
					if (_player->HasAura(ROGUE_T16_AURA_4P))
						_player->CastSpell(target, ROGUE_T16_SPELL_4P, true);
		}

		void Register()
		{
			OnHit += SpellHitFn(spell_item_rogue_t16_4p_SpellScript::HandleOnHit);
		}
	};
...
class spell_glyph_decoy : public SpellScriptLoader
{
public:
	spell_glyph_decoy() : SpellScriptLoader("spell_glyph_decoy") { }

	class spell_glyph_decoy_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_glyph_decoy_SpellScript);

		void HandleOnHit()
		{
			if (Player* _player = GetCaster()->ToPlayer())
				if (Unit* target = GetHitUnit())
					if (_player->HasAura(56800))
						_player->CastSpell(target, 112833, true);
		}

		void Register()
		{
			OnHit += SpellHitFn(spell_glyph_decoy_SpellScript::HandleOnHit);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_glyph_decoy_SpellScript();
	}
};

class spell_novo_asas : public SpellScriptLoader
{
public:
	spell_novo_asas() : SpellScriptLoader("spell_novo_asas") { }

	class spell_novo_asas_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_novo_asas_SpellScript);
		...
		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())
				...
				if (caster->HasAura(21359))
					...
					caster->CastSpell(caster, 107082, true);
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_novo_asas_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_novo_asas_SpellScript();
	}
};
...
{
	new spell_novo_asas();
	new spell_glyph_decoy();
	new spell_item_rogue_t16_2p();
