...
enum ShamanSpells
{
	...
	SHAMAN_T162PRESTO_AURA					= 145378,
	SHAMAN_T162PRESTO_SPELL					= 145379
};
...
class spell_sham_glyph_ground : public SpellScriptLoader
{
public:
	spell_sham_glyph_ground() : SpellScriptLoader("spell_sham_glyph_ground") { }

		class spell_sham_glyph_ground_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_sham_glyph_ground_SpellScript);

		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())
			{
				if (caster->HasAura(55441))
				{
					if (roll_chance_i(100))

						caster->CastSpell(caster, 89523, true);
		...
		{
			OnCast += SpellCastFn(spell_sham_glyph_ground_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_sham_glyph_ground_SpellScript();
	}
};



// Call of the Elements2 - 108285
class spell_sha_call_of_the_elements2 : public SpellScriptLoader
{
public:
	spell_sha_call_of_the_elements2() : SpellScriptLoader("spell_sha_call_of_the_elements2") { }

	class spell_sha_call_of_the_elements2_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_sha_call_of_the_elements2_SpellScript);

		void HandleOnCast()
		{
			if (Player* _player = GetCaster()->ToPlayer())
			{
				if (_player->HasAura(56218))
				{
					// immediately finishes the cooldown on totems with less than 3min cooldown
					const SpellCooldowns& cm = _player->GetSpellCooldownMap();
					for (SpellCooldowns::const_iterator itr = cm.begin(); itr != cm.end();)
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_sha_call_of_the_elements2_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_sha_call_of_the_elements2_SpellScript();
	}
};



class spell_sha_glyph_of_shear : public SpellScriptLoader
{
public:
	spell_sha_glyph_of_shear() : SpellScriptLoader("spell_sha_glyph_of_shear") { }

	class spell_sha_glyph_of_shear_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_sha_glyph_of_shear_SpellScript);
		...
		void HandleOnHit()
		{
			if (Unit* target = GetHitUnit())
				if (Player* _player = GetCaster()->ToPlayer())
					if (_player->HasAura(56218))
					if (_player->HasSpellCooldown(57994))
						_player->ReduceSpellCooldown(57994, (1 * 3000));
		}

		void Register()
		{
			OnHit += SpellHitFn(spell_sha_glyph_of_shear_SpellScript::HandleOnHit);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_sha_glyph_of_shear_SpellScript();
	}
};

class spell_visual_fantasma2 : public SpellScriptLoader
{
public:
	spell_visual_fantasma2() : SpellScriptLoader("spell_visual_fantasma2") { }

	class spell_visual_fantasma2_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_visual_fantasma2_SpellScript);
		...
		void HandleOnHit()
		{
			if (Unit* target = GetHitUnit())
				if (Player* _player = GetCaster()->ToPlayer())
					if (_player->HasAura(56218))
						if (_player->HasSpellCooldown(8177))
							_player->ReduceSpellCooldown(8177, (1 * 15000));
		}

		void Register()
		{
			OnHit += SpellHitFn(spell_visual_fantasma2_SpellScript::HandleOnHit);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_visual_fantasma2_SpellScript();
	}
};

class spell_sha_glyph_of_purge2 : public SpellScriptLoader
{
public:
	spell_sha_glyph_of_purge2() : SpellScriptLoader("spell_sha_glyph_of_purge2") { }

	class spell_sha_glyph_of_purge2_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_sha_glyph_of_purge2_SpellScript);
		...
		void HandleOnHit()
		{
			...
			if (Player* _player = GetCaster()->ToPlayer())
				...
				if (_player->HasAura(56218))
					_player->CastSpell(target, 27626, true);
		}

		void Register()
		{
			OnHit += SpellHitFn(spell_sha_glyph_of_purge2_SpellScript::HandleOnHit);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_sha_glyph_of_purge2_SpellScript();
	}
};

class spell_visual_fantasma : public SpellScriptLoader
{
public:
	spell_visual_fantasma() : SpellScriptLoader("spell_visual_fantasma") { }

	class spell_visual_fantasma_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_visual_fantasma_SpellScript);

		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())

				if (caster->HasAura(21359))

					caster->CastSpell(caster, 145945, true);
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_visual_fantasma_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_visual_fantasma_SpellScript();
	}
};

class spell_visual_raioaura : public SpellScriptLoader
{
public:
	spell_visual_raioaura() : SpellScriptLoader("spell_visual_raioaura") { }

	class spell_visual_raioaura_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_visual_raioaura_SpellScript);
		...
		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())
				...
				if (caster->HasAura(21359))
					...
					caster->CastSpell(caster, 138981, true);
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_visual_raioaura_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_visual_raioaura_SpellScript();
	}
};

class spell_visual_raioativo : public SpellScriptLoader
{
public:
	spell_visual_raioativo() : SpellScriptLoader("spell_visual_raioativo") { }

	class spell_visual_raioativo_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_visual_raioativo_SpellScript);
		...
		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())
				...
				if (caster->HasAura(21359))
					...
					caster->CastSpell(caster, 52663, true);
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_visual_raioativo_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_visual_raioativo_SpellScript();
	}
};

class spell_glyph_maelstromenhanc : public SpellScriptLoader
{
public:
	spell_glyph_maelstromenhanc() : SpellScriptLoader("spell_glyph_maelstromenhanc") { }

	class spell_glyph_maelstromenhanc_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_glyph_maelstromenhanc_SpellScript);

		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())

				if (caster->HasAura(147762))
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_glyph_maelstromenhanc_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_glyph_maelstromenhanc_SpellScript();
	}
};

class spell_sham_t16_enhanc_2p : public SpellScriptLoader
{
public:
	spell_sham_t16_enhanc_2p() : SpellScriptLoader("spell_sham_t16_enhanc_2p") { }

	class spell_sham_t16_enhanc_2p_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_sham_t16_enhanc_2p_SpellScript);
		...
		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())
				...
				if (caster->HasAura(144962))
					...
					caster->CastSpell(caster, 144901, true);
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_sham_t16_enhanc_2p_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_sham_t16_enhanc_2p_SpellScript();
	}
};



class spell_sham_t16_enhanc_4p : public SpellScriptLoader
{
public:
	spell_sham_t16_enhanc_4p() : SpellScriptLoader("spell_sham_t16_enhanc_4p") { }

	class spell_sham_t16_enhanc_4p_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_sham_t16_enhanc_4p_SpellScript);

		void HandleOnHit()
		{
			if (Unit* target = GetHitUnit())
				if (Player* _player = GetCaster()->ToPlayer())
					if (_player->HasAura(144966))
					{
						if (_player->HasSpellCooldown(114049))
						{
							if (roll_chance_i(30))
							_player->ReduceSpellCooldown(114049, (1 * 15000));
						}
					}
		}
		...
		void Register()
		{
			OnHit += SpellHitFn(spell_sham_t16_enhanc_4p_SpellScript::HandleOnHit);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_sham_t16_enhanc_4p_SpellScript();
	}
};


class spell_sham_t16_enhanc_4p_storm : public SpellScriptLoader
{
public:
	spell_sham_t16_enhanc_4p_storm() : SpellScriptLoader("spell_sham_t16_enhanc_4p_storm") { }

	class spell_sham_t16_enhanc_4p_storm_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_sham_t16_enhanc_4p_storm_SpellScript);
		...
		void HandleOnHit()
		{
			if (Unit* target = GetHitUnit())
				if (Player* _player = GetCaster()->ToPlayer())
					if (_player->HasAura(144966))
					{
						if (_player->HasSpellCooldown(17364))
						{
							if (roll_chance_i(100))
								_player->ReduceSpellCooldown(17364, (1 * 8000));
						}
					}
		}
		...
		void Register()
		{
			OnHit += SpellHitFn(spell_sham_t16_enhanc_4p_storm_SpellScript::HandleOnHit);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_sham_t16_enhanc_4p_storm_SpellScript();
	}
};

class spell_sham_t16_enhanc_4p_lash : public SpellScriptLoader
{
public:
	spell_sham_t16_enhanc_4p_lash() : SpellScriptLoader("spell_sham_t16_enhanc_4p_lash") { }

	class spell_sham_t16_enhanc_4p_lash_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_sham_t16_enhanc_4p_lash_SpellScript);
		...
		void HandleOnHit()
		{
			if (Unit* target = GetHitUnit())
				if (Player* _player = GetCaster()->ToPlayer())
					if (_player->HasAura(144966))
					{
						if (_player->HasSpellCooldown(60103))
						{
							if (roll_chance_i(60))
								_player->ReduceSpellCooldown(60103, (1 * 10000));
						}
					}
		}
		...
		void Register()
		{
			OnHit += SpellHitFn(spell_sham_t16_enhanc_4p_lash_SpellScript::HandleOnHit);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_sham_t16_enhanc_4p_lash_SpellScript();
	}
};
...
{
	new spell_visual_fantasma2();
	new spell_sham_t16_enhanc_4p_storm();
	new spell_sham_t16_enhanc_4p_lash();

	new spell_sham_t16_enhanc_4p();
	new spell_sham_t16_enhanc_2p();


	new spell_glyph_maelstromenhanc();
	new spell_visual_raioativo();
	new spell_visual_raioaura();
	new spell_visual_fantasma();
	new spell_sha_glyph_of_shear();
	new spell_sha_glyph_of_purge2();
	new spell_sha_call_of_the_elements2();

	new spell_sham_glyph_ground();
