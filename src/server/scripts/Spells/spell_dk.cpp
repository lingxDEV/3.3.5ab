            ...
            void HandleAfterHit()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        if (Aura* necroticHealAbsorb = target->GetAura(DK_SPELL_NECROTIC_STRIKE))
                        {
                            float amount = _player->GetTotalAttackPowerValue(BASE_ATTACK) * 1.00f;
...
class spell_novo_dksombralow : public SpellScriptLoader
{
public:
	spell_novo_dksombralow() : SpellScriptLoader("spell_novo_dksombralow") { }

	class spell_novo_dksombralow_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_novo_dksombralow_SpellScript);
		...
		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())
				...
				if (caster->HasAura(21359))
					...
					caster->CastSpell(caster, 72521, true);
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_novo_dksombralow_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_novo_dksombralow_SpellScript();
	}
};

class spell_novo_dksombrahigh : public SpellScriptLoader
{
public:
	spell_novo_dksombrahigh() : SpellScriptLoader("spell_novo_dksombrahigh") { }

	class spell_novo_dksombrahigh_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_novo_dksombrahigh_SpellScript);
		...
		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())
				...
				if (caster->HasAura(21359))
					...
					caster->CastSpell(caster, 72523, true);
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_novo_dksombrahigh_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_novo_dksombrahigh_SpellScript();
	}
};

class spell_rime_dk_fix : public SpellScriptLoader
{
public:
	spell_rime_dk_fix() : SpellScriptLoader("spell_rime_dk_fix") { }

	class spell_rime_dk_fix_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_rime_dk_fix_SpellScript);

		void HandleOnCast()
		{
			if (Unit* caster = GetCaster())
			{
				if (caster->HasAura(59057))
				{
					if (roll_chance_i(40))
						caster->CastSpell(caster, 59052, true);
				}
			}
		}
		void Register()
		{
			OnCast += SpellCastFn(spell_rime_dk_fix_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_rime_dk_fix_SpellScript();
	}
};
void AddSC_deathknight_spell_scripts()
{
    new spell_dk_death_and_decay();
	new spell_rime_dk_fix();
	new spell_novo_dksombrahigh();
	new spell_novo_dksombralow();
