            ...
            void HandleAfterCast()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (_player->getLevel() == 90 && _player->HasAura(HUNTER_T16_2P_BONUS))
                        _player->ReduceSpellCooldown(HUNTER_SPELL_RAPID_FIRE, 8000);
                }
            }
...
class spell_hun_lock_n_load_novo : public SpellScriptLoader
{
public:
    spell_hun_lock_n_load_novo() : SpellScriptLoader("spell_hun_lock_n_load_novo") { }

    class spell_hun_lock_n_load_novo_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_hun_lock_n_load_novo_AuraScript);
		...
		void OnTick(AuraEffect const* aurEff)
        {
            if (Player* _player = GetCaster()->ToPlayer())
            {
                    ...
                    if (roll_chance_i(20))
					_player->CastSpell(_player, 56453, true);
        ...
        void Register()
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_hun_lock_n_load_novo_AuraScript::OnTick, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_hun_lock_n_load_novo_AuraScript();
    }
};


class spell_novo_petclone : public SpellScriptLoader
{
public:
	spell_novo_petclone() : SpellScriptLoader("spell_novo_petclone") { }

	class spell_novo_petclone_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_novo_petclone_SpellScript);
		...
		void HandleOnCast()
		{
			...
			if (Unit* caster = GetCaster())
				...
				if (caster->HasAura(21359))
					...
					caster->CastSpell(pet, 45204, true);
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_novo_petclone_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_novo_petclone_SpellScript();
	}
};

void AddSC_hunter_spell_scripts()
{
	new spell_novo_petclone();
    new spell_hun_glyph_of_aspect_of_the_beast();
	new spell_hun_lock_n_load_novo();
