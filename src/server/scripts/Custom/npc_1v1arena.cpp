...
public:
	...
	bool OnGossipHello(Player* player, Creature* me)
	{
		...
		if (player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
			player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Remover Lista de 1 vs 1", GOSSIP_SENDER_MAIN, 3, "Tem certeza?", 0, false);
		else
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Entrar na fila de Arena 1 vs 1", GOSSIP_SENDER_MAIN, 20);
		...
		case ROLES_HEALER:
		case ROLES_TANK:
			ChatHandler(player->GetSession()).PSendSysMessage("Não pode entrar com especialização de TANK OU HEALER!");
			player->CLOSE_GOSSIP_MENU();
			break;
		default:
			ChatHandler(player->GetSession()).PSendSysMessage("Precisa escolher uma especialização antes!");
		}
		return true;
	}
	...
	bool OnGossipSelect(Player* player, Creature* me, uint32 /*uiSender*/, uint32 uiAction)
	{
		...
		case 20: // Join Queue Arena (unrated)
		{
			if (JoinQueueArena(player, me, false) == false)
				ChatHandler(player->GetSession()).SendSysMessage("Algo não ocorreu como planejado.");
